document.querySelector("#mycartbtn").addEventListener("click",gotocart);
function gotocart(){
      window.location.pathname="zappos-repository/card/cart.html"
      // window.location.href="cart.html";
}
document.querySelector("#goto").addEventListener("click",go);
function go(){
      window.location.pathname="zappos-repository/mainsign/signin.html"
      //     window.location.href="signin.html";
}
document.querySelector(".bar2").addEventListener("click",women);
function women(){
      window.location.pathname="zappos-repository/women/women.html";
      // window.location.href="women.html";
}
document.querySelector(".bar1").addEventListener("click",dummy);
function dummy(){
      window.location.pathname="zappos-repository/dummy/dummy.html";
}
document.querySelector(".bar3").addEventListener("click",dummy);
function dummy(){
      window.location.pathname="zappos-repository/dummy/dummy.html";
}
document.querySelector(".bar4").addEventListener("click",kid);
function kid(){
      window.location.pathname="zappos-repository/kids/kids.html";
}
document.querySelector(".bar5").addEventListener("click",dummy);
function dummy(){
      window.location.pathname="zappos-repository/dummy/dummy.html";
}
document.querySelector(".bar6").addEventListener("click",dummy);
function dummy(){
      window.location.pathname="zappos-repository/dummy/dummy.html";
}
document.querySelector(".bar7").addEventListener("click",dummy);
function dummy(){
      window.location.pathname="zappos-repository/dummy/dummy.html";
}
document.querySelector(".bar8").addEventListener("click",dummy);
function dummy(){
      window.location.pathname="zappos-repository/dummy/dummy.html";
}

    
   